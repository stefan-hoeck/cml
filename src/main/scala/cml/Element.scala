...
object Element {
  case object Du extends Element(0)
  ...
  val values = List[Element](
    Du, H, He, Li, Be, B, C, N, O, F, Ne, Na, Mg, Al, Si, P, S, Cl, Ar, K, Ca,
  ...
  private[this] val symbolMap: Map[String,Element] =
    values map (e ⇒ e.symbol.toLowerCase → e) toMap

  def fromNr(i: Int): Element = nrMap(i)

  def fromSymbol(s: String): Option[Element] =
    symbolMap get s.toLowerCase

  def fromSymbolV (s: String): ValRes[Element] = {
    def msg = s"Unknown element: $s"
